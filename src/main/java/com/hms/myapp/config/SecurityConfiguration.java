...
import com.hms.myapp.security.*;
import com.hms.myapp.security.oauth2.AudienceValidator;
import com.hms.myapp.security.oauth2.CustomClaimConverter;
import com.hms.myapp.web.filter.SpaWebFilter;
...
public class SecurityConfiguration {
    ...
    // Hardcoded to avoid OIDC discovery at startup (which fails without internet).
    // This must match the Google OIDC issuer.
        ...
        http.cors(withDefaults())
            ...
            .authorizeHttpRequests(authz ->
                ...
                authz
                    ...
                    .requestMatchers(org.springframework.http.HttpMethod.GET, "/api/rooms/available").permitAll()
                    .requestMatchers(org.springframework.http.HttpMethod.GET, "/api/rooms/{id}").permitAll()
                    .requestMatchers("/api/rooms/**").hasAnyAuthority(AuthoritiesConstants.ADMIN, AuthoritiesConstants.RECEPTIONIST)
                    .requestMatchers("/api/bookings/**").hasAnyAuthority(AuthoritiesConstants.ADMIN, AuthoritiesConstants.RECEPTIONIST, AuthoritiesConstants.USER)
                    .requestMatchers("/api/guests/**").hasAnyAuthority(AuthoritiesConstants.ADMIN, AuthoritiesConstants.RECEPTIONIST, AuthoritiesConstants.USER)
    ...
    OAuth2UserService<OidcUserRequest, OidcUser> oidcUserService() {
        ...
        return userRequest -> {
            ...
            return new DefaultOidcUser(oidcUser.getAuthorities(), oidcUser.getIdToken(), oidcUser.getUserInfo(), userNameAttributeName);
        };
    }
    ...
    JwtDecoder jwtDecoder(ClientRegistrationRepository clientRegistrationRepository, RestTemplateBuilder restTemplateBuilder) {
        // Use the well-known Google JWK Set URI directly so the key fetch is lazy
        // (only when the first JWT arrives), avoiding a startup timeout when the
