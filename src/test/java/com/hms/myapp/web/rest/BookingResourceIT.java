...
import static com.hms.myapp.web.rest.TestUtil.sameNumber;
...
class BookingResourceIT {
    ...
    private static final LocalDate UPDATED_CHECK_IN_DATE = LocalDate.parse("2026-08-28");

    private static final LocalDate DEFAULT_CHECK_OUT_DATE = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_CHECK_OUT_DATE = LocalDate.parse("2026-08-28");
    ...
    private static final Instant UPDATED_CREATED_DATE = Instant.ofEpochMilli(1787912193534L);

    private static final String DEFAULT_CANCELLED_REASON = "AAAAAAAAAA";
    private static final String UPDATED_CANCELLED_REASON = "BBBBBBBBBB";
    ...
    public static Booking createEntity(EntityManager em) {
        Booking booking = new Booking()
            ...
            .cancelledReason(DEFAULT_CANCELLED_REASON);
        // Add required entity
        Guest guest;
        if (TestUtil.findAll(em, Guest.class).isEmpty()) {
            guest = GuestResourceIT.createEntity(em);
            em.persist(guest);
            em.flush();
        } else {
            guest = TestUtil.findAll(em, Guest.class).get(0);
        }
    ...
    public static Booking createUpdatedEntity(EntityManager em) {
        Booking updatedBooking = new Booking()
            ...
            .numberOfGuests(UPDATED_NUMBER_OF_GUESTS)
            .specialRequests(UPDATED_SPECIAL_REQUESTS)
            .createdDate(UPDATED_CREATED_DATE)
            .cancelledReason(UPDATED_CANCELLED_REASON);
        // Add required entity
        Guest guest;
        if (TestUtil.findAll(em, Guest.class).isEmpty()) {
            guest = GuestResourceIT.createUpdatedEntity(em);
            em.persist(guest);
            em.flush();
        } else {
            guest = TestUtil.findAll(em, Guest.class).get(0);
        }
        updatedBooking.setGuest(guest);
        // Add required entity
        Room room;
        if (TestUtil.findAll(em, Room.class).isEmpty()) {
    ...
    @Test
    @Transactional
    void checkNumberOfGuestsIsRequired() throws Exception {
        long databaseSizeBeforeTest = getRepositoryCount();
        // set the field null
    ...
    void getAllBookings() throws Exception {
        ...
        restBookingMockMvc
            ...
            .andExpect(jsonPath("$.[*].cancelledReason").value(hasItem(DEFAULT_CANCELLED_REASON)));
    }
    ...
    void getBooking() throws Exception {
        ...
        restBookingMockMvc
            ...
            .andExpect(jsonPath("$.cancelledReason").value(DEFAULT_CANCELLED_REASON));
    }
    ...
    void partialUpdateBookingWithPatch() throws Exception {
        ...
        partialUpdatedBooking.status(UPDATED_STATUS).createdDate(UPDATED_CREATED_DATE).cancelledReason(UPDATED_CANCELLED_REASON);
