...
/**
 * REST controller for managing {@link com.hms.myapp.domain.Room}.
 */
@RestController
...
public class RoomResource {
    ...
    public ResponseEntity<List<RoomDTO>> getAvailableRooms(
        ...
        @RequestParam(required = false) com.hms.myapp.domain.enumeration.RoomType roomType
    ) {
        LOG.debug("REST request to get available Rooms between {} and {}", checkIn, checkOut);
        if (checkIn == null || checkOut == null) {
            throw new BadRequestAlertException("Check-in and check-out dates are required", ENTITY_NAME, "missingdates");
        }
        if (!checkOut.isAfter(checkIn)) {
            throw new BadRequestAlertException("Invalid date range", ENTITY_NAME, "invaliddates");
        }
        return ResponseEntity.ok(roomService.findAvailableRooms(checkIn, checkOut, roomType));
    }
