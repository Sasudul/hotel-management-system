          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('paidDate')}>
                  Paid Date <FontAwesomeIcon icon={getSortIconByFieldName('paidDate')} />
                </th>
                <th className="hand" onClick={sort('notes')}>
                  Notes <FontAwesomeIcon icon={getSortIconByFieldName('notes')} />
                </th>
                <th>
                  Booking <FontAwesomeIcon icon="sort" />
                </th>
                <th>
            ...
            <tbody>
              {paymentList.map(payment => (
                <tr key={`entity-${payment.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/payment/${payment.id}`} variant="link" size="sm">
                      {payment.id}
                    </Button>
                  </td>
                  ...
                  <td>{payment.paidDate ? <TextFormat type="date" value={payment.paidDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{payment.notes}</td>
                  <td>{payment.booking ? <Link to={`/booking/${payment.booking.id}`}>{payment.booking.id}</Link> : ''}</td>
                  <td>{payment.recordedBy ? payment.recordedBy.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/payment/${payment.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
                      ...
                      <Button
                        onClick={() =>
                          (globalThis.location.href = `/payment/${payment.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        variant="danger"
                        size="sm"
